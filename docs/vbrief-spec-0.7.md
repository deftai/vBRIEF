...
- **PlanItem `planRefs` field** — Array of plan reference URIs for container items aggregating multiple external vBRIEFs (§4.4).
...
- **Reference type registry** — Canonical `x-vbrief/*` types for common reference kinds (Appendix B).
- **`failed` status** — Added to the Status enum (§5.1). Already present in v0.6 schema; now normative in spec prose.

## Status of This Document

This document specifies a beta release of the vBRIEF format. It is intended for early implementers and feedback. Comments and issues should be filed at <https://github.com/deftai/vBRIEF/issues>.
...
| Field | Type | Requirement | Description |
|-------|------|-------------|-------------|
| `version` | string | REQUIRED | Format version. MUST be `"0.7"` for this specification. |
...
The `version` field MUST be the string `"0.7"` for this specification. Implementations MUST reject documents with unrecognized version strings or MAY handle them with a clear warning.
...
| `items` | array | Nested array of PlanItem objects. Preferred in v0.7 for hierarchical plans. |
...
vBRIEF v0.6 standardizes nested PlanItem collections on the `items` field. Implementations SHOULD emit `items` when writing v0.6 documents.
...
The `planRefs` field is an OPTIONAL array of URI strings, using the same schemes as `planRef`. It allows container items (`group`, `milestone`, `epic`) to reference multiple external vBRIEFs without requiring one subItem per reference:
...
```json
{
  ...
  "status": "auto",
  "planRefs": [
    "file://./pending/task-a.vbrief.json",
    "file://./pending/task-b.vbrief.json"
  ]
}
```
...
Consumer tokens MUST be lowercase ASCII letters, digits, and hyphens. Consumer tokens SHOULD be registered in the consumer registry at `vbrief.org/registry` to prevent collision; unregistered tokens are valid but discouraged.
...
Extension properties MAY appear at the document root, inside `vBRIEFInfo`, inside `plan`, inside `plan.metadata`, inside any value of `narratives`, inside any `PlanItem`, and inside any other object permitted by this specification.
...
The following consumer tokens are reserved by this specification and MUST NOT be used by external consumers: `vbrief`, `core`, `spec`, `internal`.

The `x-vbrief/` prefix used by `references[].type` is a closed enum administered by this specification (see [Appendix B](#appendix-b-reference-type-registry)) and is not an open extension namespace.
...
A document is vBRIEF v0.7 conformant if and only if:

1. The root object contains a `vBRIEFInfo` field with `version` equal to `"0.7"`.
...
5. Implementations SHOULD emit `items` when writing v0.7 documents and MAY emit `subItems` only for backward compatibility.
...
- **[vBRIEF JSON Schema]** — [../schemas/vbrief-core-0.7.schema.json](../schemas/vbrief-core-0.7.schema.json)
...
The normative JSON Schema for vBRIEF v0.7 is located at [`schemas/vbrief-core-0.7.schema.json`](../schemas/vbrief-core-0.7.schema.json).
...
The `x-vbrief/` namespace for `VBriefReference.type` is a closed enum administered by this specification. The following canonical types are defined:

| Type | Description | Required fields |
|------|-------------|----------------|
| `x-vbrief/plan` | Reference to another vBRIEF plan document | `uri` |
| `x-vbrief/github-issue` | GitHub issue | `uri`, `title` |
| `x-vbrief/github-pr` | GitHub pull request | `uri`, `title` |
| `x-vbrief/commit` | Git commit | `uri` (git SHA or URL) |
| `x-vbrief/external` | Arbitrary external resource | `uri` |
| `x-vbrief/research` | Research artifact or document | `uri`, `title` |
| `x-vbrief/adr` | Architecture Decision Record | `uri`, `title` |

Custom reference types SHOULD use the `x-<consumer>/` namespace (e.g. `x-myapp/ticket`) to avoid collision with the canonical registry. The `x-vbrief/` namespace is reserved for spec-defined types only.

**Note:** `x-vbrief/` is the current unregistered namespace. A future `vnd.vbrief.` IANA registration path exists if the specification is submitted to IANA for formal standardization.
...
### C.2 Structured Plan with Container Types (v0.7)
